...
from transformers import Trainer, TrainingArguments, BertTokenizer, BertForMaskedLM, IntervalStrategy, \
    RobertaTokenizer, RobertaForMaskedLM, EarlyStoppingCallback
...
def create_trainer(wrapper, train_dataset, val_dataset, checkpoint_dir, batch_size, epoch_num):
    ...
    def compute_metrics(pred):
        inputs = pred.inputs # (batch_size, seq_len) 原始输入, 里面存有[MASK]的token_id
        mask_index = np.where(inputs == wrapper.preprocessor.pvp.mask_id)[1] # (batch_size) 取出每条文本中[MASK]位置的索引
        labels = [pred.label_ids[i, indices] for i, indices in enumerate(mask_index)] # (batch_size) 取出每条文本中[MASK]位置的真实token_id
        b_indices = np.array([(i, j) for i, j in enumerate(mask_index)])
        preds = pred.predictions[b_indices[:, 0], b_indices[:, 1], :].argmax(-1) # (batch_size) 取出每条文本中[MASK]位置的vocab_len个token中，概率最大的那个id
        # 根据Verbalizer的映射集重新处理一下
        for i in range(len(preds)):
            for j,x in enumerate(wrapper.preprocessor.pvp.mlm_logits_to_cls_logits_tensor):
                if preds[i] in x:
                    preds[i] = wrapper.preprocessor.pvp.label_list_id[j]
    ...
    class CustomTrainer(Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            # 运行模型
            new_inputs = {k:v for k, v in inputs.items() if k not in ['mlm_labels', 'label_idx']}
            outputs = model(**new_inputs)

            # 求布尔向量 方便从logits、labels中提取mask位置上的数据
            mlm_bool = inputs['mlm_labels'] > 0

            # 计算verbalizer中每个标签的平均得分
            ...
            # 计算mask位置上的损失 单独拿出来算是因为，这个还可以改为多标签分类的损失函数
            log_probs = -nn.functional.log_softmax(outputs['logits'][mlm_bool], dim=-1) # [32, 30522]
            nll_loss = log_probs.gather(dim=-1, index=inputs['labels'][mlm_bool].unsqueeze(-1)) # [32, 1]
            mlm_loss = nll_loss.sum() / nll_loss.size()[0] # []
            ...
            # 计算联合损失
            loss = 0.5 * mlm_loss + 0.5 * auxiliary_loss
...
def main():
    ...
    """
    DATASETS = ['example_zh','example_en','eprstmt_zh','agnews_en']
    ...
    # Params:
        num_examples: 数据集乱序后取num_examples条数据出来. 只有当num_examples严格效于该数据集总条数时才会乱序.
        num_examples_per_label: 可选[int, List]. int: 每种标签都取num_examples_per_label条文本. List: 每种标签取List中对应条文本.
    """
    train_data = load_examples(config['task_name'], config['data_dir'], set_type="train", num_examples = None, num_examples_per_label = 300)
    dev_data = load_examples(config['task_name'], config['data_dir'], set_type="dev", num_examples = None, num_examples_per_label = 100)
    test_data = load_examples(config['task_name'], config['data_dir'], set_type="test", num_examples = None, num_examples_per_label = 200)
    """
